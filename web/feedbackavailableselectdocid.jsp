                ...
                <div class="head-title">
                    <div class="left">
                        ...
                        <ul class="breadcrumb">
                            ...
                            <li>
                                <a class="active" href="#">Available Feedback</a>
                            </li>
                        </ul>
                    </div>
                </div>
                    ...
                    <div class="order">
                        <div class="head">
                            ...
                            <form action="MainController" method="POST">
                                ...
                                <button type="submit" name="action" value="ViewAvailableFeedbackByDoctorID" style="border: none; font-size: 18px; cursor: pointer;"><i class='bx bx-filter' ></i></button>
                            </form>
                        </div>
                        <br>
                        ...
                        <c:if test="${LIST_AVAILABLE_COMMENT_BY_DOCTORID != null}">
                            <table>
                                <thead>
                                    <tr>
                                        <th>No.</th>
                                ...
                                <tbody>
                                    <c:forEach var="listAvailableCommentByDoctorID" items="${LIST_AVAILABLE_COMMENT_BY_DOCTORID}" varStatus="loop">
                                    <form action="MainController" method="POST">
                                        <tr>
                                        <input type="hidden" value="${listAvailableCommentByDoctorID.commentID}" name="commentID"/> 
                                        <td>${loop.count}</td>
                                        <td>
                                            <p style="font-weight: 600; color: #113355;">${listAllAvailableComment.name}</p>
                                        </td>
                                        <td>${listAvailableCommentByDoctorID.cmtTime}</td>
                                        <td style="font-weight: 500; color: #2eb7e5;"><span>${listAvailableCommentByDoctorID.contentCmt}</span></td>
                                        <td><span class="status process" style="color: black;">${listAvailableCommentByDoctorID.rate}</span></td>
                                        <td><span class="status completed">${listAvailableCommentByDoctorID.status}</span></td>
                                            <c:if test="${listAvailableCommentByDoctorID.doctorID == 1}">
                                            <td><span>Calvin Carlo</span></td>
                                        </c:if>
                                        <c:if test="${listAvailableCommentByDoctorID.doctorID == 2}">
                                            <td><span>Cristino Murphy</span></td>
                                        </c:if>
                                        <c:if test="${listAvailableCommentByDoctorID.doctorID == 3}">
                                            <td><span>Alia Reddy</span></td>
                                        </c:if>
                                        <c:if test="${listAvailableCommentByDoctorID.doctorID == 4}">
                                            <td><span>Toni Kovar</span></td>
                                        </c:if>
                                        <c:if test="${listAvailableCommentByDoctorID.doctorID == 5}">
                                            <td><span>Jessica McFarlane</span></td>
                                        </c:if>
                                        <c:if test="${listAvailableCommentByDoctorID.doctorID == 6}">
                                            <td><span>Elsie Sherman</span></td>
                                        </c:if>
                                        <c:if test="${listAvailableCommentByDoctorID.doctorID == 7}">
                                            <td><span>Bertha Magers</span></td>
                                        </c:if>
                                        <c:if test="${listAvailableCommentByDoctorID.doctorID == 8}">
                                            <td><span>Louis Batey</span></td>
                                        </c:if>
                                        <td><button type="submit" name="action" value="UpdateAvailableFeedback" style="border: none; font-size: 25px; color: green; cursor: pointer;"><i class='bx bx-export'></i></button></td>
