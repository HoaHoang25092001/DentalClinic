                    ...
                    <div class="order">
                        ...
                        <c:if test="${LIST_ALL_AVAILABLE_COMMENT != null}">
                            <table>
                                ...
                                <tbody>
                                    <c:forEach var="listAllAvailableComment" items="${LIST_ALL_AVAILABLE_COMMENT}" varStatus="loop">
                                    <form action="MainController" method="POST">
                                        <tr>
                                        <input type="hidden" value="${listAllAvailableComment.commentID}" name="commentID"/> 
                                        <td>${loop.count}</td>
                                        <td>
                                            <p style="font-weight: 600; color: #113355;">${listAllAvailableComment.name}</p>
                                        </td>
                                        <td>${listAllAvailableComment.cmtTime}</td>
                                        <td style="font-weight: 500; color: #2eb7e5;"><span>${listAllAvailableComment.contentCmt}</span></td>
                                        <td><span class="status process" style="color: black;">${listAllAvailableComment.rate}</span></td>
                                        <td><span class="status completed">${listAllAvailableComment.status}</span></td>
                                            <c:if test="${listAllAvailableComment.doctorID == 1}">
                                            <td><span>Calvin Carlo</span></td>
                                        </c:if>
                                        <c:if test="${listAllAvailableComment.doctorID == 2}">
                                            <td><span>Cristino Murphy</span></td>
                                        </c:if>
                                        <c:if test="${listAllAvailableComment.doctorID == 3}">
                                            <td><span>Alia Reddy</span></td>
                                        </c:if>
                                        <c:if test="${listAllAvailableComment.doctorID == 4}">
                                            <td><span>Toni Kovar</span></td>
                                        </c:if>
                                        <c:if test="${listAllAvailableComment.doctorID == 5}">
                                            <td><span>Jessica McFarlane</span></td>
                                        </c:if>
                                        <c:if test="${listAllAvailableComment.doctorID == 6}">
                                            <td><span>Elsie Sherman</span></td>
                                        </c:if>
                                        <c:if test="${listAllAvailableComment.doctorID == 7}">
                                            <td><span>Bertha Magers</span></td>
                                        </c:if>
                                        <c:if test="${listAllAvailableComment.doctorID == 8}">
